...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, image_path, mask_path, mask_mode, target_size, augment=True, training=True, mask_reverse = False):
        ...
        self.data = self.load_flist(image_path)
        self.mask_data = self.load_flist(mask_path)
    ...
    def load_flist(self, flist):
        ...
        if isinstance(flist, str):
            if os.path.isdir(flist):
                flist = list(glob.glob(flist + '/*.jpg')) + list(glob.glob(flist + '/*.png')) + list(glob.glob(flist + '/*.JPG')) + list(glob.glob(flist + '/*/*/*.jpg'))
                flist.sort()
                return flist
