...
for i, data in enumerate(dataset): 
    with torch.no_grad(): 
            ...
            for vv in range(vert_ver):
                for i in range(256):
                    for j in range(256):
                        ...
                        np_tmp_seg[i, j, 0] = np_resized_seg[vv + vert_ver*j, hv + hori_ver*i, 0] 
                        np_tmp_seg[i, j, 1] = np_resized_seg[vv + vert_ver*j, hv + hori_ver*i, 1] 
                        np_tmp_seg[i, j, 2] = np_resized_seg[vv + vert_ver*j, hv + hori_ver*i, 2] 

                img_arr.append(np.copy(np_tmp_img)) 
                msk_arr.append(np.copy(np_tmp_msk)) 
                seg_arr.append(np.copy(np_tmp_seg)) 
        ...
        for i in range(len(img_arr)):
            ...
            seg = Image.fromarray(seg_arr[i]) 

            img_90 = img.rotate(90) 
            msk_90 = msk.rotate(90) 
            seg_90 = seg.rotate(90) 

            img_180 = img.rotate(180) 
            msk_180 = msk.rotate(180) 
            seg_180 = seg.rotate(180) 

            img_270 = img.rotate(270) 
            msk_270 = msk.rotate(270) 
            seg_270 = seg.rotate(270) 

            img_flp = img.transpose(method=Image.FLIP_LEFT_RIGHT)
            msk_flp = msk.transpose(method=Image.FLIP_LEFT_RIGHT)
            seg_flp = seg.transpose(method=Image.FLIP_LEFT_RIGHT) 

            compltd_img, reconst_img, lr_x = model(mskimg_transform(img).unsqueeze(0), msk_transform(msk).unsqueeze(0), seg_transform(seg).unsqueeze(0)) 
            compltd_img_90, reconst_img_90, lr_x_90 = model(mskimg_transform(img_90).unsqueeze(0), msk_transform(msk_90).unsqueeze(0), seg_transform(seg_90).unsqueeze(0)) 
            compltd_img_180, reconst_img_180, lr_x_180 = model(mskimg_transform(img_180).unsqueeze(0), msk_transform(msk_180).unsqueeze(0), seg_transform(seg_180).unsqueeze(0)) 
            compltd_img_270, reconst_img_270, lr_x_270 = model(mskimg_transform(img_270).unsqueeze(0), msk_transform(msk_270).unsqueeze(0), seg_transform(seg_270).unsqueeze(0)) 
            compltd_img_flp, reconst_img_flp, lr_x_flp = model(mskimg_transform(img_flp).unsqueeze(0), msk_transform(msk_flp).unsqueeze(0), seg_transform(seg_flp).unsqueeze(0)) 
