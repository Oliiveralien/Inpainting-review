...
class OurModel(BaseModel):
    ...
    def initialize(self, opt): 
        ...
        input_nc = opt.input_nc ### masked_img + mask (RGB + gray) 4 channels 
    ...
    def encode_input(self, masked_image, mask, real_image, infer=False): 
        ...
        return input_msked_img, input_msk, real_image 

    def encode_input_test(self, masked_image, mask, infer=False): 
        input_msked_img = masked_image.data.cuda()
        input_msk = mask.data.cuda() 
        ...
        return input_msked_img, input_msk 
    ...
    def forward(self, masked_img, mask, real_img=None, infer=False, mode=None): 
        # inference 
        if mode == 'inference': 
            compltd_img, reconst_img, lr_x = self.inference(masked_img, mask) 
            return compltd_img, reconst_img, lr_x 
    
        # Encode inputs 
        input_msked_img, input_msk, real_image = self.encode_input(masked_img, mask, real_img) 

        # Fake Generation 
        compltd_img, reconst_img, lr_x, lr_img = self.netG.forward(input_msked_img, input_msk, real_image) 
        lr_msk = input_msk
    ...
    def inference(self, masked_img, mask): 
        # Encode inputs 
        input_msked_img, input_msk = self.encode_input_test(Variable(masked_img), Variable(mask), infer=True) 

        # Fake Generation 
        if torch.__version__.startswith('0.4'): 
            with torch.no_grad(): 
                compltd_img, reconst_img, lr_x, lr_img = self.netG.forward(input_msked_img, input_msk) 
        else: 
            compltd_img, reconst_img, lr_x, lr_img = self.netG.forward(input_msked_img, input_msk)
        return compltd_img, reconst_img, lr_x 
...
class InferenceModel(OurModel): 
    def forward(self, inp1, inp2): 
        masked_img = inp1 
        mask = inp2 
        return self.inference(masked_img, mask) 
