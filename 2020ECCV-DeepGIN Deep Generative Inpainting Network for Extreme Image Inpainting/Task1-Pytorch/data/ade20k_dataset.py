...
class ADE20kDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if opt.phase == 'train':
            ...
            self.B_seg_paths, _ = make_dataset_with_conditions(self.dir_A, '_seg')
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == 'train':
            ...
            new_A = self.rotate(new_A) 
            new_A = self.ensemble(new_A) 
    ...
    def resize_or_crop(self, img, seg_img, method=Image.BICUBIC):
        w, h = img.size
        new_w = w
        new_h = h
